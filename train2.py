...
def plot_random_samples(X):
    ...
    plot_samples(None, samples, scale=1.0, scatter=False,
                 symm_axis=None, lw=1.2, alpha=.7, c='k', fname='samples')
...
@click.command(help="Train Airfoil Gan")
@click.option("--latent_dim", type=click.INT, default=3,
              help="Number of latend dimensions.")
@click.option("--noise_dim", type=click.INT, default=10,
              help="Number of noise dimensions.")
@click.option("--bezier_degree", type=click.INT, default=31,
              help="Bezier Degree")
@click.option("--train_steps", type=click.INT, default=20000,
              help="Train Steps")
@click.option("--batch_size", type=click.INT, default=32,
              help="Batch Size")
def run(latent_dim, noise_dim, bezier_degree):
